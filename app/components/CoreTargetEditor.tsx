...
import { targetPolicy, targetError, targetConfirmation, RULE_ZH, RULE_EN } from "../../miniprogram/utils/target-policy";
...
export function CoreTargetEditor({ profile, onUpdated }: { profile: LifeProfile; onUpdated: (profile: LifeProfile) => void }) {
  ...
  return <section className="settings-card locked-profile-card">
    <div className="settings-title-row"><h2>{en ? "Core life scale" : "核心余生刻度"}</h2><span>{en ? "Up to 7 changes" : "累计最多修改 7 次"}</span></div>
    <p>{en ? RULE_EN : RULE_ZH}</p>
    <dl><div><dt>{en ? "Birth date (fixed)" : "出生日期（不可修改）"}</dt><dd>{profile.birth_date}</dd></div><div><dt>{en ? "Target date" : "目标日期"}</dt><dd>{profile.target_date}</dd></div><div><dt>{en ? "Target age" : "目标年龄"}</dt><dd>{profile.target_age || (en ? "Exact date" : "具体日期")}</dd></div><div><dt>{en ? "Total changes left" : "累计剩余修改次数"}</dt><dd>{policy.configured ? policy.remaining + " / 7" : "—"}</dd></div>{policy.firstYear ? <div><dt>{en ? "First-year changes left" : "首年剩余修改次数"}</dt><dd>{policy.firstYearRemaining} / 3</dd></div> : null}</dl>
    <p className="upload-hint">{!policy.configured ? (en ? "Adjustment status unavailable. Refresh before changing your target." : "暂时无法读取调整资格，请刷新后重试。") : !policy.remaining ? (en ? "All 7 changes have been used." : "累计 7 次调整机会已用完。") : policy.canChange ? (en ? "You can adjust your target now." : "现在可以调整目标。") : (en ? "Next available: " : "下次可调整：") + nextAt}</p>
