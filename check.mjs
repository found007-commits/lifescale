...
for (const [name, snippets] of pages) {
  ...
  for (const snippet of snippets) {
    if (!contents.includes(snippet)) throw new Error(`${name} 缺少：${snippet}`);
  }
}

const server = await readFile(resolve(root, "server.js"), "utf8");
for (const route of ["/app/", "/privacy", "/terms", "/third-parties", "/account-deletion", "/healthz"]) {
  if (!server.includes(route)) throw new Error(`server.js 缺少路由：${route}`);
}

console.log("LifeScale 扁平目录页面与路由检查通过。");
