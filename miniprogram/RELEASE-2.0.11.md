...
- **小程序 2.0.8 三页的实际渲染验收仍未做**：自动化通道已验证可用（开发者工具自动化端口可连、首页在 iPhone 12/13 Pro 视口下正常渲染、0 异常），但圣所三页需要**已登录的小程序会话**，未登录时 `requireSession()` 会按设计把深链重定向到首页。必须先由人扫一次码登录，之后可由脚手架一键抓完三页。
