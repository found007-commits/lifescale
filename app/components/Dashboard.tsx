...
import { MediaViewer as EntryGallery } from "./MediaViewer";
...
export function Dashboard({ session, initialLocale }: { session: Session; initialLocale: Locale }) {
  ...
  const [sharingEntry, setSharingEntry] = useState<LifeEntry | null>(null);
  ...
  async function remove(entry: LifeEntry) {
    if (!window.confirm(en ? "Delete this entry, its media and notes? This cannot be undone." : "确定删除这条记录及其影像和留言吗？此操作无法撤销。")) return;
  ...
  return (
    <main className="app-shell" key={locale} ref={surfaceRef} lang={locale === "zh-TW" ? "zh-TW" : locale === "zh" ? "zh-CN" : "en"}>
      ...
      {sharingEntry ? <EntryShareDialog entry={sharingEntry} locale={locale} onClose={() => setSharingEntry(null)} /> : null}
      {toast ? <div className="toast" role="status">{toast}</div> : null}
    </main>
  );
}

function EntryCard({ entry, locale, onShare }: { entry: LifeEntry; locale: Locale; onShare: (entry: LifeEntry) => void }) {
  const en = locale === "en";
