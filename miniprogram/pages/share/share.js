...
Page({
  data: { layout: "separate", hasPhoto: false, busy: true, saving: false, cards: [], selected: 0, error: "" },
  onLoad() {
    ...
    this.getOpenerEventChannel().on("entry", ({ entry, locale }) => {
      ...
      this.setData({ hasPhoto: Boolean(entry.imageUrl) });
  ...
  async generate() {
    ...
    try {
      ...
      this.canvas = canvas;
      if (this.entry.imageUrl && !this.photo) {
        const info = await call("getImageInfo", { src: this.entry.imageUrl });
        this.photo = await new Promise((resolve, reject) => {
      ...
      const plan = planCard(canvas.getContext("2d"), content, this.photo, this.data.layout);
      const cards = [];
      for (let i = 0; i < plan.pages.length; i++) {
        if (this.closed) return;
        drawCard(canvas, this.photo, this.entry, plan, i, this.locale);
