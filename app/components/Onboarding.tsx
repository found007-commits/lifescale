...
export function Onboarding({ session, initialLocale, onComplete }: { session: Session; initialLocale: Locale; onComplete: (profile: LifeProfile) => void }) {
  ...
  async function finish() {
    if (!accepted || !confirmed || !validTarget || !gender || !session.user.email) return;
    ...
    <main className="onboarding-shell" key={locale} ref={surfaceRef} lang={locale === "zh-TW" ? "zh-TW" : locale === "zh" ? "zh-CN" : "en"}>
      ...
      {step === 0 ? (
        <section className="onboarding-welcome">
          <div className="onboarding-welcome-copy"><p className="kicker">WELCOME TO LIFESCALE</p><h1>{en ? "See the life ahead." : "看见余生，"}<em>{en ? " Make today count." : "认真今天。"}</em></h1><p>{en ? "LifeScale is a life-time account for the days you live, the time beyond your goal, and the memories you choose to preserve. It never predicts death. It helps you see time, keep today, and protect what matters." : "余生有刻是一款贯穿生前记录、生命加时与身后纪念的生命时间账户。它不预测死亡，只帮助你看见时间、留下今天，并把重要的事好好保存。"}</p><div className="welcome-privacy-promise"><strong>{en ? "Private by default, from the first record" : "从第一条记录开始，默认仅你可见"}</strong><p>{en ? "There is no creator or operations console for browsing or editing private records. Your birth date stays fixed; your target can be adjusted once a full year has passed, up to 3 times." : "产品内没有供创作者或运营人员浏览、修改私密记录的内容后台；出生日期不可修改，目标每满一年可调整一次、累计最多 3 次。"}</p></div><button className="primary-button large" onClick={() => setStep(1)}>{en ? "I understand. Set my scale" : "我理解，开始设定"}</button></div>
          <div className="onboarding-values"><article><span>01</span><h2>{en ? "Private by default" : "默认仅本人可见"}</h2><p>{en ? "Your records are not published and are isolated by account permissions." : "记录不公开，并通过账号权限与其他用户隔离。"}</p></article><article><span>02</span><h2>{en ? "Adjust thoughtfully" : "目标可审慎调整"}</h2><p>{en ? "Your birth date stays fixed. Your target may be adjusted after a full year, up to 3 times in total. Initial setup does not count; each adjustment starts a new one-year wait." : "出生日期确认后不可修改；目标年龄每满一年可调整一次，累计最多 3 次。首次设定不占次数，每次调整后重新等待一年。"}</p></article><article><span>03</span><h2>{en ? "Leave completely" : "可注销并彻底删除"}</h2><p>{en ? "Export your data or permanently delete the account when you choose." : "你可以导出资料，也可以在决定离开时永久注销账号。"}</p></article></div>
        </section>
      ) : null}

      {step === 1 ? (
        <section className="onboarding-form-page">
          <div className="onboarding-form-intro"><p className="kicker">IMPORTANT</p><h1>{en ? "Take your time." : "请认真填写，"}<em>{en ? " This defines your scale." : "这会定义你的刻度。"}</em></h1><p>{en ? "Your birth date stays fixed. Your target may be adjusted after a full year, up to 3 times in total. Initial setup does not count; each adjustment starts a new one-year wait." : "出生日期确认后不可修改；目标年龄每满一年可调整一次，累计最多 3 次。首次设定不占次数，每次调整后重新等待一年。"}</p></div>
          <div className="onboarding-card">
            <div className="critical-notice"><strong>{en ? "Choose thoughtfully" : "请认真选择"}</strong><p>{en ? "Your birth date stays fixed. Your target may be adjusted after a full year, up to 3 times in total. Initial setup does not count; each adjustment starts a new one-year wait." : "出生日期确认后不可修改；目标年龄每满一年可调整一次，累计最多 3 次。首次设定不占次数，每次调整后重新等待一年。"}</p></div>
            ...
            <GenderSelector locale={locale} value={gender} onChange={setGender} />
      ...
      {step === 2 && metrics ? (
        <section className="onboarding-confirm-page">
          <div className="confirm-heading"><p className="kicker">FINAL CONFIRMATION</p><h1>{en ? "This is your goal," : "这是你的目标，"}<em>{en ? " not your fate." : "不是命运。"}</em></h1><p>{en ? "Check your first setting carefully. After the target date, LifeScale enters Bonus Time and never marks you as deceased automatically." : "请认真核对你的首次设定。到达目标日期后，LifeScale 会进入“生命加时”，绝不会自动把你标记为死亡。"}</p></div>
          <div className="onboarding-card confirm-card"><div className="confirm-scale"><div><strong>{metrics.remainingDays.toLocaleString()}</strong><span>{en ? "days remaining" : "剩余天数"}</span></div></div><dl><div><dt>{en ? "Gender" : "性别"}</dt><dd>{gender ? genderLabel(gender, locale) : ""}</dd></div><div><dt>{en ? "Birth date" : "出生日期"}</dt><dd>{formatLocalizedDate(birthDate, locale)}</dd></div><div><dt>{targetType === "age" ? (en ? "Target age" : "目标年龄") : (en ? "Target method" : "目标方式")}</dt><dd>{targetType === "age" ? `${numericTargetAge}${en ? " years" : " 岁"}` : (en ? "Exact date" : "具体日期")}</dd></div><div><dt>{en ? "Target date" : "目标日期"}</dt><dd>{formatLocalizedDate(resolvedTargetDate, locale)}</dd></div><div><dt>{en ? "Target lock" : "目标锁定"}</dt><dd>{en ? "One year between changes · 3 total" : "每满一年一次 · 累计 3 次"}</dd></div></dl><div className="critical-notice final"><strong>{en ? "Check once more" : "请再次确认"}</strong><p>{en ? "Your birth date stays fixed. Your target may be adjusted after a full year, up to 3 times in total. Initial setup does not count; each adjustment starts a new one-year wait." : "出生日期确认后不可修改；目标年龄每满一年可调整一次，累计最多 3 次。首次设定不占次数，每次调整后重新等待一年。"}</p></div><label className="confirmation-check"><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} /><span>{en ? "I have checked my details and understand the one-year interval and 3-adjustment limit." : "我已逐项核对，并理解每满一年可调整一次、累计最多 3 次的规则。"}</span></label>{error ? <p className="form-status">{error}</p> : null}<div className="onboarding-actions"><button className="change-email" onClick={() => setStep(1)}>{en ? "Edit" : "返回修改"}</button><button className="primary-button" disabled={!confirmed || busy} onClick={finish}>{busy ? (en ? "Saving…" : "正在保存…") : (en ? "Confirm and enter LifeScale" : "确认并进入 LifeScale")}</button></div></div>
        </section>
      ) : null}
    </main>
  );
